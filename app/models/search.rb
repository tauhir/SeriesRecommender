...
class Search < ApplicationRecord
  ...
  def create_series_list(ext_series, language: "en_US", list_type: "search")
    ext_series = [ext_series] unless ext_series.is_a?(Array)
    SeriesList.new(
      language: language,
  ...
  def get_recommended
    ...
    return nil unless get_liked # should display a section telling users to start liking to get started
    liked = get_liked.get_list
    disliked = get_disliked.try(:get_list)
    ...
    recommend_list = SeriesList.where(list_type: "recommended").find_by(search_id: id)
    if recommend_list
      recommend_list.set_series(recommends_results)
    else
      recommend_list = create_series_list(recommends_results, list_type: "recommended")
    end
    recommend_list
  ...
  def get_liked
    SeriesList.where(list_type: "liked").find_by(search_id: id)
  end

  def get_disliked
    SeriesList.where(list_type: "disliked").find_by(search_id: id)
  end
  ...
  def complete_search
    ...
    query_hash = {'query': current_query}
  ...
  def post_search
    if results > 0
      ...
      return if current_query == "popular page"
      byebug
